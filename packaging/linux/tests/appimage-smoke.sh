#!/usr/bin/env bash
set -euo pipefail
artifact="${1:-dist/FURY-0.5.0-linux-x86_64.AppImage}"
expected_version="${2:-${DUBL_VERSION:-0.5.0}}"
...
[[ "$version" == "FURY $expected_version — DUBL 3.69" ]] || { echo "unexpected version: $version" >&2; exit 1; }
